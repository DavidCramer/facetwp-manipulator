!(function ($) {
    ...
    $.fn.fwpmanipLayout = function (opts) {
        ...
        return this.each(function () {
            ...
            grid.addClass('_fwpmanip_grid_init');

            // setup loading
            grid.html('<div class="fwpmanip-loading" style=""></div>');
            ...
            var init_builder = function () {
                ...
                // remove loader
                grid.find('.fwpmanip-loading').remove();
            }

            var create_component = function (type, data) {

                var component = $('<div class="fwpmanip-component component-' + type + '" data-type="' + type + '" data-label="' + type + '"><div class="fwpmanip-component-toolbar"><span class="dashicons dashicons-admin-generic fwpmanip-component-edit"></span> ' + type + '<span class="dashicons dashicons-no remove-component right"></span></div></div>'),
            ...
            var remove_column = function (column) {
                ...
                var contents = column.find('.fwpmanip-component'),
                ...
                for (var r = 0; r < rows.length; r++) {
                    ...
                    for (var c = 0; c < columns.length; c++) {
                        ...
                        var column = $(columns[c]),
                            component_struct = [],
                            components = column.find('> .column-container > .fwpmanip-component');
            ...
            //  add element
            grid.on('click', '.fwpmanip-grid-element', function () {
                var clicked = $(this),
            ...
            grid.on('click', '.remove-component', function () {
                var component = $(this).closest('.fwpmanip-component');
                if (confirm('Really?')) {
            ...
            grid.on('click', '.remove-row', function () {
                var row = $(this).closest('.row'),
                    contents = row.find('.fwpmanip-component');
                if (contents.length) {
            ...
            grid.on('click', '.fwpmanip-component-edit', function () {
                var clicked = $(this),
                    component = clicked.closest('.fwpmanip-component'),
                    template_ele = $( '#' + grid.prop('id') + '-' + component.data('type') + '-tmpl' ),
                    template = Handlebars.compile( template_ele.html() );

                var component_modal = $.fwpmanipModal({
            ...
            grid.on('click', '.add-component', function () {
                ...
                current_modal = $.fwpmanipModal({
                    modal: 'fwpmanip_component',
                    title: 'Insert',
                    width : 550,
                    height: 445,
                    focus: true,
                ...
                $( current_modal.content ).on('click', '.fwpmanip-component-trigger', component_editor );
            ...
            function init_rows() {
                ...
                grid.sortable({
                    //handle : '.fwpmanip-row-toolbar',
                    //axis: "y",
                    connectWith: ".fwpmanip-control-layout",
                    delay: 150,
                    ...
                    helper: function (e, item) {
                        return $('<div class="fwpmanip-row-helper" style="width: 155px; height: 30px;"></div>');
                    },
                });

                sort_column_container = grid.find(".column-container").sortable({
                    //handle : '.fwpmanip-component-toolbar',
                    delay: 150,
                    forceHelperSize: false,
                    items: '.fwpmanip-component',
                    appendTo: "body",
                    connectWith: ".column-container",
                    cursorAt: {top: 35, left: 74},
                    zIndex: 1000000,
                    helper: function (e, item) {
                        return $('<div class="fwpmanip-component-helper" style="width: 155px; height: 30px;" data-type="' + item.data('type') + '" data-label="' + item.data('label') + '">' + item.data('label') + '</div>');
                    },
                    update: function () {
                ...
                grid.find('.row').each(function () {
                    ...
                    sort_column = row.find(".column").droppable({
                        accept: '.fwpmanip-fwpmanipbuilder-layout-layout_grid-first_row-0-settings-conditions-mini',
                        greedy: true,
            ...
            /// components
            $(".fwpmanip-fwpmanipbuilder-layout-layout_grid-first_row-0-settings-conditions-mini").draggable({
                cursor: "move",
                appendTo: "body",
                cursorAt: {top: 15},
                helper: function (event) {
                    var item = $(event.target);
                    return $('<div class="fwpmanip-component-helper" data-type="' + item.data('type') + '" data-label="' + item.data('label') + '">' + item.html() + '</div>');
                }
            });
    ...
    $(window).load(function () {
        $(document).on('fwpmanip.init', function () {
            $('.fwpmanip-control-layout').not('._fwpmanip_grid_init').fwpmanipLayout();
        });
    });
