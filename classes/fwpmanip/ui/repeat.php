...
class repeat extends panel {
	...
	public function set_assets() {
		...
		$this->assets['script']['repeat'] = $this->url . 'assets/js/repeat' . FWPMANIP_ASSET_DEBUG . '.js';
		$this->assets['style']['repeat']  = $this->url . 'assets/css/repeat' . FWPMANIP_ASSET_DEBUG . '.css';
	...
	public function prepare_data() {
		$submit_data = fwpmanip()->request_vars( 'post' );
	...
	public function render() {
		...
		$output = '<div data-instance="{{json this}}" data-fwpmanip-template="' . esc_attr( $this->id() ) . '" ' . $this->build_attributes() . '>';
	...
	public function render_repeatable() {
		...
		$output = '<div class="fwpmanip-repeat">';
		$output .= $this->render_template();
		if ( ! empty( $this->child ) ) {
			$output .= $this->render_children();
		}

		$output .= '<button type="button" class="button button-small fwpmanip-remover"><span class="dashicons dashicons-no"></span></button> </div>';
	...
	public function render_repeatable_more() {
		...
		$this->instance  = 0;
		$output          = '<div class="repeatable-footer"><button type="button" class="button" data-fwpmanip-repeat="' . esc_attr( $this->id() ) . '">' . esc_html( $label ) . '</button></div>';
	...
	protected function set_active_styles() {
		...
		$style = '#' . $this->id() . ' .fwpmanip-repeat{ box-shadow: 1px 0 0 ' . $this->base_color() . ' inset, -37px 0 0 #f5f5f5 inset, -38px 0 0 #ddd inset, 0 2px 3px rgba(0, 0, 0, 0.05); };';
